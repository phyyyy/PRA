...
import numpy as np
import models
from utils import SelectedCifar10, Normalize, replace_relu_with_AS
import PIL
import timm
...
parser.add_argument('--save_dir', type=str, default='adv_cifar10/PRA/')
parser.add_argument('--target_attack', default=False, action='store_true')
parser.add_argument(
    '--layer_config',
    type=str,
    default='./configs/cifar10_vgg19_layers.yaml',
...
def _prep_hook(model, hooks, MD_pos_list, sigma, ori_img, iteration, device, decay_values):
    ...
    with torch.no_grad():
        for layer_pos in MD_pos_list:
            md_module = model[1].features.module
            md_module = md_module[int(layer_pos)]
            ...
            decay_rate = decay_values.get(str(layer_pos), 1.0)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(
        torch.load('./your_ckpt', map_location=device)['state_dict']
    )
    ...
    for ind, (ori_img, label) in enumerate(ori_loader):
        ...
        for i in range(niters):
            ...
            if 'mdi2fgsm' in method or 'mifgsm' in method:
                input_grad = 1 * m + input_grad / torch.norm(
                    input_grad, dim=(1, 2, 3), p=1, keepdim=True
                )
                m = input_grad
        ...
        np.save(
            save_dir + '/adv_batch_{}.npy'.format(ind),
            torch.round(img.data * 255).cpu().numpy().astype(np.uint8())
        )
        np.save(
            save_dir + '/ori_batch_{}.npy'.format(ind),
