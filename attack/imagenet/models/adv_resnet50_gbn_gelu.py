...
global_val_gbn_forward_time = 4
class NoOpAttacker():
    def attack(self, image, model):
        return image
...
if global_val_gbn_forward_time == 8:
    ...
    def forward(self, x):
        if self.sing: 
            ...
            if training:
                self.eval()
                self.apply(to_adv)
                if global_val_gbn_forward_time == 4:
                    if self.bn_num == 0:
                        self.apply(to_0)
                    elif self.bn_num == 1:
                        self.apply(to_1)
                        ...
                        self.apply(to_3)
                elif global_val_gbn_forward_time == 8:
                    if self.bn_num == 0:
                        self.apply(to_0)
                    elif self.bn_num == 1:
                        self.apply(to_1)
                ...
                if isinstance(self.attacker, NoOpAttacker):
                    ...
                    aux_images, _ = self.attacker.attack(x, self._forward_impl, True, True, self.mixup_fn)
                    images =  aux_images
                self.train()
                self.apply(to_clean)
                ...
                if isinstance(self.attacker, NoOpAttacker):
                    ...
                    aux_images, _ = self.attacker.attack(x, self._forward_impl, True, False, False)
                    images =  aux_images
                return self._forward_impl(images)
